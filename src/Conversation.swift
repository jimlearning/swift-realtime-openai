...
public final class Conversation: ObservableObject, @unchecked Sendable {
    ...
    private var queuedSamplesCancellable: AnyCancellable?
    ...
    private init(client: RealtimeAPI) {
        ...
        // 订阅 queuedSamples 的变化，自动同步 isPlaying
        queuedSamplesCancellable = queuedSamples.$array
            .receive(on: DispatchQueue.main)
            .sink { [weak self] arr in
                Task { @MainActor in
                    self?.isPlaying = !arr.isEmpty
                }
            }
    }
    ...
    deinit {
        ...
        Task { [playerNode, audioEngine] in
            await Self.cleanUpAudio(playerNode: playerNode, audioEngine: audioEngine)
        }
    }
...
extension Conversation {
    ...
    @MainActor public func startListening() throws {
        ...
        Task.detached { [audioEngine] in
            audioEngine.inputNode.installTap(
                onBus: 0, bufferSize: 4096, format: audioEngine.inputNode.outputFormat(forBus: 0)
            ) { [weak self] buffer, _ in
                Task { [weak self] in
                    await self?.processAudioBufferFromUser(buffer: buffer)
                }
            }
        }
    ...
    @MainActor public func startHandlingVoice() throws {
    ...
    public static func cleanUpAudio(playerNode: AVAudioPlayerNode, audioEngine: AVAudioEngine) {
